...
The traditional algebraic notion of $\ZZ$-torsor yields a category {\color{red}whose objects are the $\ZZ$-torsors.
...
Due to univalence, the type $\color{red}\TorZ$ of all $\ZZ$-torsors
is a connected pointed type whose fundamental group is $\ZZ$; we may call it the {\em classifying type} of $\ZZ$.  
In this paper we show that $\color{red}\TorZ$ behaves the way a circle ought to behave, 
...
%
Voevodsky chose not to include higher inductive types, nor inductive types\footnote{\color{red}{\em UniMath} accepts just a few specific types
...
%
Secondly, Voevodsky planned to prove consistency of {\sl UniMath} in a series of papers\footnote{\color{red}Early work \cite{1211.2851} with
...
and we give an alternative induction principle in \cref{sec:integers-induction}.
...
{\color{red}Recall that for a group $G$, a {\em $G$-torsor} consists of a nonempty set $X$ and
...
is a bijection commuting with $s$. We evaluate this bijection {\color{red}at} $0$.
Let $\ev_0(p) \defeq \pr_1(p)_*(0)$ for all $p : \pt =_\TorZ \pt$,
then $\ev_0 : (\pt =_\TorZ \pt) \to \zet$.  Combining these observations {\color{red}and}
...
\footnote{\color{red}This construction was formalised by Grayson 
...
and the goal is the to produce from the point $a$ and the path $l$ over $\Sloop$,
...
in \cite[6.9]{hottbook}, then the path $r : f(\pt)= a$ above is a reflexivity path.
It then follows
...
judgmentally equal.  Thus the right side of the equation \ref{eq:circle-induction} can be replaced by $l$.
...
{\color{red}where $\Prop$ is the type of all propositions (in a chosen universe).}
...
Proofs are by (nested) induction as indicated in the text,
...
$0 \defeq \zzero$
\item $\zpos: \NN \to \zet$ for positive {\color{red}integers},
$1 \defeq \zpos(0),\ldots$.
\item $\zneg: \NN \to \zet$ for negative {\color{red}integers}, 
...
{\color{red}Denoting the successor function on $\NN$ by $S$,
...
{\color{red}which sends $p$ to $\pr_1(p)_*(0)$}.
...
The function $h:\prod_{z:\ZZ}(P(z)\color{red})$ thus defined satisfies
...
{\color{red}In this section, pairs $(X,f)$ will be of type $\sum_{X:\UU}(X\to X)$.}
...
{\color{red}The formalization of \cref{sec:TorZ-recursion} can be found in \cite{circlerec-Dan},
...
type of $\ZZ$-torsors, but is more parsimonious.\footnote{\color{red}Our formalization, however, uses the traditional definition.}
...
(compare with \cite[6.9]{hottbook}, {\color{red}$\trunc\_: A\to\Trunc A$ is 
...
\footnote{\color{red}
Recall that in {\UniMath} $\Trunc T \defeq \prod_{P:\Prop}((T \to P) \to P)$, and 
the function $\trunc\_: T\to\Trunc T$ is
defined by $\trunc t  (P,f) \defeq f(t)$ for all $P: \Prop$, $f: T\to P$, and $t:T$.
Thus we naturally get a recursion principle: if $P: \Prop$, then any
$g: T \to P$ defines an $f : \Trunc T \to P$ by setting $f(x)\defeq x(P,g)$.
Clearly $f$ satisfies $f(\trunc t) \jdeq \trunc t (P,g) \jdeq g(t)$, for all $t: T$.

For induction the situation is different: if $P: \Trunc T \to \Prop$,
then any $g: \prod_{t:T} P(\trunc t)$ again defines an
$f:  \prod_{x:\Trunc T} P(x)$. Since the latter type is a proposition,
there is only one such $f$ and it can be obtained in several ways. 
One way is the following. Let $x: \Trunc T$. Then $P(x): \Prop$. 
Define $h_x: T \to P(x)$ by mapping $t$ to the transport of $g(t): P(\trunc t)$ 
along the path of type $\trunc t = x$ coming from the proof that $\Trunc T$ is a proposition.
By the recursion principle, $h_x$ defines $h'_x : \Trunc T \to P(x)$.
Now set $f(x) \defeq h'_x(x)$. Indeed $f: \prod_{x:\Trunc T} P(x)$. However,
because of the transport, we don't have $f(\trunc t) \jdeq g(t)$,
as the path of type $\trunc t = \trunc t$ coming from the proof that $\Trunc T$ is 
a proposition need not be the path given by reflexivity.

There are several other ways of defining $f$, but none of them
satisfies the computation rule $f(\trunc t)\jdeq g(t)$. It is currently
...
{\color{red}The aim of this section is to show that $\TorZ$ satisfies the induction principle \cref{eq:circle-induction} for the circle,
thereby establishing that $\TorZ$ has the defining property of the circle.
The recursion principle is the non-dependent version of the induction principle, namely that there is a function of the following type:
\begin{equation}
...
To be able to apply \cref{lem:dep-elim-TorZ}, we need to find a suitable proposition.  {\color{red}The idea is to find a
  correspondence\footnote{\color{red}A {\em correspondence} from a type $T$ to a type $T'$ is a type $C$ with projections $pr_1 : C \to T$ and
...
\draw[->,dotted] (m-2-1) to[bend left=60] node[midway,left]{$\scriptstyle\vec c_p$} (m-1-1) ;
\draw[->,dotted] (m-2-1) -- (m-1-2) node[midway,below]{$\scriptstyle c_p$} ;
...
{\color{red}For $T := (X,f,t) : \TorZ$, let $\vec{c}_p(T)$ denote the center of contractibility of $Q_p(X,f)$ as constructed in the proof above.
...
{\color{red}We make extensive use of the functions and lemmas from \cref{sec:pathovers}.}
...
{\color{red}We make extensive use of this simplification}.
...
After a decade of further work, this interpretation has now been extended to include a class of model categories presenting all $(\infty,1)$-topos by Shulman~\cite{shulman:univinj}, and made into an interpretation of the entire formal system by Brunerie, de Boer, Lumsdaine, and M\"{o}rtberg~\cite{initiality}.
Thus, we can now say conclusively that the result of this paper yields a theorem about all $(\infty,1)$-toposes.
...
One way to obtain this from our theorem would be to observe that according to the interpretation of higher inductive types in higher toposes constructed by~\cite{1705.07088}, the higher inductive $\Sc$ is a presentation of the above homotopy colimit.
...
However, a more direct approach is also possible, which avoids discussing higher inductive types at all:\footnote{As of this writing, higher inductive types are not yet included in the work of~\cite{initiality}.  Also the models of universes in~\cite{shulman:univinj} are not yet known to be closed under parametrized higher inductive types, although this is not a problem in our instance since $\Sc$ is not parametrized.} we can use our theorem to show that the interpretation of $\TorZ$ in an $(\infty,1)$-topos has the universal property of the homotopy colimit that defines a circle object.
Therefore, since it classifies $\ZZ$-torsors, so does any other such homotopy colimit.

Most of the work needed for this approach inside of type theory has already been done by Sojakova~\cite{sojakova:hits-hias}.
...
Now suppose we have a Quillen model category $\cE$ that presents an $(\infty,1)$-topos $\sE$.
To show that $\TorZ$ has the universal property of a circle object in $\sE$, we must show that for any object $A$ of $\sE$, the diagram of hom-spaces ($\infty$-groupoids)
...
It is also worth saying a few words about why our $\TorZ$ ``classifies $\ZZ$-torsors'' in the $(\infty,1)$-categorical sense, i.e., that for any object $A$, the hom-space $\sE(A,\TorZ)$ is naturally equivalent to the $\infty$-groupoid of small $\ZZ$-torsors in $\sE/A$.
Importantly, the relevant notion of ``$\ZZ$-torsor'' is the topos- and sheaf-theoretic one: a $\ZZ$-torsor $X$ in $\sE/A$ need not be ``globally'' isomorphic to $\ZZ\times A$, but only ``locally'' so in the sense that there is an effective epimorphism $p:B \twoheadrightarrow A$ such that $p^*X$ is isomorphic over $B$ to $\ZZ\times B$.

To start with, we observe that by the univalence axiom, $\UU$ is an object classifier, in the sense that $\sE(A,\UU)$ is naturally equivalent, by pullback of the canonical map $\UUp\to \UU$, to a full sub-$\infty$-groupoid of the core of $\sE/A$ (the fiberwise ``small'' objects).
...
Similarly, $\sum_{X:\UU} (X\to X)$ is the exponential in $\sE/\UU$ of $\UU_p$ by itself, so by the pullback-stability and universal property of exponentials, the space of lifts of $f:A\to \UU$ to $\sum_{X:\UU} (X\to X)$ is equivalent to the space of endomorphisms of the corresponding object $f^*\UUp$ of $\sE/A$.
Thus, $\sum_{X:\UU} (X\to X)$ classifies objects equipped with an endomorphism, in that $\sE(A, \sum_{X:\UU} (X\to X))$ is naturally equivalent to the $\infty$-groupoid of small objects of $\sE/A$ equipped with an endomorphism.

Finally, we need to consider the topos-theoretic interpretation of propositional truncation.
...
Interpreted fiberwise in an $(\infty,1)$-topos, this says that if $X\to A$ is any map then $\Trunc{X}\to A$ is a reflection into the sub-$(\infty,1)$-category of $\sE/A$ consisting of the monomorphisms, i.e., maps $P\to A$ whose diagonal $P\to P\times_A P$ is an equivalence.
By~\cite[Example 5.2.8.16 and Corollary 6.5.1.14]{lurie:higher-topoi} (in the case $n=-1$), the effective epimorphisms and monomorphisms in an $(\infty,1)$-topos form a factorization system, and hence in particular the map $X\to \Trunc{X}$ is an effective epimorphism.

Now recall the definition of $\TorZ$ as $\sum_{(X,f)}\Trunc{(\zet,s)=(X,f)}$.
This means we have an (effective epimorphism, monomorphism) factorization $W \twoheadrightarrow \TorZ \rightarrowtail \sum_{X:\UU} (X\to X)$, where $W = \sum_{(X,f)} (\zet,s)=(X,f)$ is (by arguments like those above) a classifier for objects $X$ equipped with an endomorphism $f$ and a \emph{specified} equivalence to $\zet$ respecting the endomorphisms.
...
This means that when the classifying map $A\to \sum_{X:\UU} (X\to X)$ is composed with $p$, it lifts to $W$; so we have the following diagram:
\[
\begin{tikzcd}
  B \ar[dd,->>] \ar[r] & W \ar[d,->>] \\
  & \TorZ \ar[d,>->] \\
...
We have proved, {\color{red}for any type family $A\to\TorZ$,} 
