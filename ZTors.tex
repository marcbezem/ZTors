...
%
Voevodsky chose not to include higher inductive types, nor inductive types\footnote{\emph{UniMath} accepts just a few specific types
...
to \emph{UniMath} for two chief reasons.
%
...
%
Secondly, Voevodsky planned to prove consistency of \emph{UniMath} in a series of papers\footnote{Early work \cite{1211.2851} with
...
\item[Homotopy initiality]
  ...
  from $\mathcal C$ to any other circle algebra $\mathcal A\jdeq(A,a,l)$
  ...
  \begin{equation}\label{eq:circle-hom}
    ...
    \sum_{f : C \to A}\sum_{r:f(c)=a} \ap{f}(s)=r \ct l \ct r^{-1}.
  \end{equation}
...
\item[Induction principle]
  ...
  where $\mathcal A\jdeq (A,a,l)$ with $A : C \to \UU$, $a : A(c)$
  and $l : a=^A_s a$, and the type of algebra sections is
  \begin{equation}\label{eq:circle-sect}
    \Ssect(\mathcal A) \defeq
    \sum_{f: \prod_{z:C} A(z)}
    \sum_{r: f(c) = a}
    {\apd{f}(s) = r\cto l \cto r^{-1}}.
  \end{equation}
...
\item[Dependent universal property]
  ...
  has a section.
\end{description}
From these equivalences it also follows that the type of induction terms
for $\mathcal C$,
\begin{equation}
...
with a path $r:f(\pt)=a$ such that $\ap{f}(\Zloop)=l$.
...
Our construction uses the propositional truncation operation $\Trunc{-}$,
as in \cite[6.9]{hottbook},
but we do not require that the dependent eliminator
witnessing the induction principle of $\Trunc{-}$
computes judgmentally on the point constructor $\trunc{-}: X \to \Trunc X$.
In the \emph{UniMath} formalization, propositional truncation is constructed as
...
in \emph{UniMath}, the non-dependent circle eliminator that we construct for $\TorZ$
...
Its application to $A,a,l$ of appropriate types
is a triple $(f,r,q)$ where $f$ is a section of $A$ and $r : f(\pt)= a$
becomes a \emph{reflexivity path}, so $f(\pt)\jdeq a$.
...
Thus the right side of the final equation in \eqref{eq:circle-induction}
...
\begin{definition}\label{def:proptruncind}
  ...
  and we are given $g : \prod_{a:A} B(\trunc a)$, then there is a function $f : \prod_{x:\Trunc A} B(x)$.
...
$f:  \prod_{x:\Trunc A} B(x)$, as follows.  (Since the latter type is a proposition,
there is only one such $f$.)
Suppose we have an element $x: \Trunc A$.  Then $B(x)$ is a proposition.
Define $h_x: A \to B(x)$ by sending $a$ to the transport of $g(a): B(\trunc a)$
along the path of type $\trunc a = x$ coming from the proof that $\Trunc A$ is a proposition.
By the recursion principle, from $h_x$ we may obtain a function $h'_x : \Trunc A \to B(x)$.
Now set $f(x) \defeq h'_x(x)$, with $f: \prod_{x:\Trunc A} B(x)$.
We have $f(\trunc a) = g(a)$, since $B(\trunc a)$ is a proposition.
