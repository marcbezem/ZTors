...
exists a bijection $e$ that makes the diagram above commute, by transporting the structure of $Z$ as a trivial $\ZZ$-torsor along $e$,
thereby expressing $e$ as an isomorphism of $\ZZ$-torsors.  Independence of the structure on $X$ from the choice of $e$ is established by
observing that $n \cdot x = f^n(x)$.
...
\begin{proof}
  ...
  \begin{align*}
    \psi_+(p) & := \sum_{h_+ : \prod_{n:\NN}P(n+1)}  \bigl((h_+(0) = f_0(p))         \\
    & \hspace {8em} \times \prod_{n:\NN}h_+(n+1) = f_n(h_+(n))\bigr) \\
    \psi_-(p) & := \sum_{h_- : \prod_{n:\NN}P(-n-1)} \bigl((h_-(0) = f_{-1}^{-1}(p))  \\
