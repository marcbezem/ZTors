...
one specific object  -- namely $\ZZ$ itself -- is the canonical $\ZZ$-torsor, whose automorphism group is $\ZZ$.}
...
{\color{red}We have formalized the result in \UniMath{}, a name which refers both to Voevodsky's (univalent) foundation of mathematics based on a formal
...
$\pt : \Sc$, a path $\Sloop : \pt = \pt$, an induction principle (for defining functions from it), and nothing more.  Adding higher inductive
types to the system would give another construction of the circle, equivalent to the one described above.  Voevodsky chose not to include higher
inductive types to {\em UniMath} for two chief reasons.  Firstly, it was (and still is) not clear how to specify precisely what a definition of
a higher inductive type consists of: see \cite{1705.07088,HoTT-Agda-HIT} for two approaches.  Secondly, Voevodsky planned to prove consistency
of {\sl UniMath} in a series of papers\footnote{Early work \cite{1211.2851} with Kapulkin and Lumsdaine to establish consistency left him
  unsatisfied, so he embarked on his series of papers, to be realized with more details exposed.} and realized that adding higher inductive
types to the system would dramatically increase the length of the series and the burden of writing them; as it was, he died before completing
the project.  The approach in this series is to interpret each of the basic constructions of the formal system as a traditional mathematical
construction: a context is interpreted as a fibrant simplicial set, a type in a context is interpreted as a fibration of fibrant simplicial
sets, an element of a type is interpreted as a section of such a fibration, and so on.  At his death the series was well underway but far from
complete: \cite{103,109,VV-relmonad,112,39,MR3607209,MR3607210}.
...
{\color{red} Recall that a $\ZZ$-torsor \cite{XYZ} consists of a set $X$ and 
a $\ZZ$-action ${\cdot} : (\zet\times X) \to X$ that is free and transitive. 
This means that for all $x,y$ in $X$ there is a unique integer $n$ such
that $n\cdot x = y$. By fixing $x$, say as $x_0$, we get a bijection $e: \zet\to X$. 
Furthermore, since $\ZZ$ has one generator, the $\ZZ$-action is completely 
determined  by the action of the integer $1$.
This action of $1$ is a bijection $X\to X$ which we denote by $f$.
Then we have $e(s(n)) = (1+n)\cdot x_0 = 1\cdot(n\cdot x_0) = f(e(n))$ for all
...
\begin{tikzpicture} %\large
   ...
   [matrix of math nodes, row sep=2em, column sep=2em, ampersand replacement=\&]
    {\zet \& \zet \\ X \& X \\ };
\draw[->](m-1-1) -- (m-1-2) node[midway,above] {$s$};
\draw[->] (m-1-1) -- (m-2-1) node[midway,left] {$e$};
\draw[->] (m-1-2) -- (m-2-2) node[midway,right] {$e$};
\draw[->] (m-2-1) -- (m-2-2) node[midway,above] {$f$};
\end{tikzpicture}
\]
Conversely, we get a $\ZZ$-torsor from any pair $(X,f)$ such that the diagram
above commutes, and this is independent of the choice of $x_0$.

With the above considerations in mind we consider pairs $(X,f)$ of type $\sum_{X:\UU}(X\to X)$,}
and introduce \emph{the pointed type of $\zet$-torsors} by adopting the following definitions
(cf.~\cref{def:TorZ}).
\begin{align*}
...
{\color{red}Let's denote the successor function on $\NN$ by $S$,}
...
{\color{red}mapping $p$ to $\pr_1(p)_*(0)$}.
...
\begin{proof}
 ...
 {\color{red} 
  \begin{align*}  
    \varphi^{-1}(p)
    &\equiv \psi_+(p) \times \psi_-(p),\text{~where $\psi_+(p),\psi_-(p)$ are}
 \end{align*}
 \begin{align*}
  \sum_{h_+ : \prod_{n:\NN}P(n+1)} \bigl(  (h_+(0) = f_0(p))  &
                          \times \prod_{n:\NN}h_+(n+1) = f_n(h_+(n))\bigr),\text{and}\\
  \sum_{h_- : \prod_{n:\NN}P(-n-1)} \bigl(   (h_-(0) = f_{-1}^{-1}(p))  &
                          \times \prod_{n:\NN}h_-(n+1) = f_{-n-2}^{-1}(h_-(n))\bigr),
  \end{align*} repectively.
The type  $\varphi^{-1}(p)$ is contractible, because $\psi_+(p)$ and
$\psi_-(p)$ are  contractible,}
...
\footnote{\color{red}
Recall that in {\UniMath} $\Trunc T \defeq \prod_{P:\Prop}((T \to P) \to P)$.
The function $\trunc\_: T\to\Trunc T$ is in {\UniMath} not a constructor,
but defined by $\trunc t  (P,f) \defeq f(t)$, for all $P: \Prop$, $f: T\to P$, and $t:T$.
...
a proposition need not be a reflexivity path.
...
{\color{red}The aim of this section is to show that $\TorZ$ satisfies a recursion principle
and an induction principle.
Together they establish that $\TorZ$ has the universal property of the circle.
The precise statement of the induction principle has been stated in the
introduction, see \cref{eq:circle-induction}.
The recursion principle is the non-dependent version of the induction principle in 
\cref{eq:circle-induction}, namely that there is a function of the following type:
\begin{equation}
...
{\color{red}where $\vec c_p(X,f)$ is a dependent function.}
