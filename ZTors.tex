...
judgmentally equal.
Thus, in this case, the function $f$ provided by the induction principle
satisfies the equation $\apd{f}(\Zloop) = l$,
which is the equation in \eqref{eq:circle-induction} with
the right-hand side replaced by $l$.
...
(compare with \cite[6.9]{hottbook}, {$\trunc\_: A\to\Trunc A$ is 
...
{For $T := (X,f,t) : \TorZ$, let $\vec{c}_p(T)$ denote the center of contractibility of $Q_p(X,f)$ as constructed in the proof above.
